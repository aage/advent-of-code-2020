module Day4Tests
    ...
    let ``validate password mandatory data works as expected`` (data:string) (expected:bool) =

        let actual = validateMandatoryData data
    ...
    [<InlineData("hcl:dab227 iyr:2012
        ecl:brn hgt:182cm pid:021572410 eyr:2020 byr:1992 ci", false)>]     
    ...
    [<InlineData("hcl:#888785
        ...
        eyr:2022", true)>]
    [<InlineData("iyr:2010 hgt:158cm hcl:#b6652a ecl:blu byr:1944 eyr:2021 
        pid:093154719", true)>]
    let ``validate password valid data recognizes invalid passwords`` (data:string) (expected:bool) =

        let r = validateCorrectData data
        let actual =
            match r with
            | Ok _ -> (true, "")
            | Error e -> (false, e)
